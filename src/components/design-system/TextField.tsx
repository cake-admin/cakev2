...
const getTextFieldColors = (state, theme, isFocused, isHovered, hasValue = false) => {
  const isDark = theme === TEXTFIELD_THEMES.DARK;
  
  if (isDark) {
    // Dark theme colors
    if (state === TEXTFIELD_STATES.DISABLED) {
      return {
    ...
    if (state === TEXTFIELD_STATES.ERROR) {
      return {
    ...
    if (state === TEXTFIELD_STATES.FOCUSED) {
      return {
    ...
    if (state === TEXTFIELD_STATES.SUCCESS) {
      return {
    ...
    if (state === TEXTFIELD_STATES.REQUIRED_EMPTY) {
      return {
        border: '#9CA3AF',
        background: '#F1F5F9',
        text: '#475569',
...
const TextField = forwardRef<HTMLInputElement, TextFieldProps>(({
  ...
  const getVisualState = () => {
    ...
    if (disabled) return TEXTFIELD_STATES.DISABLED;
    if (error) return TEXTFIELD_STATES.ERROR;
    if (success) return TEXTFIELD_STATES.SUCCESS;
    if (isFocused) return TEXTFIELD_STATES.FOCUSED; // Prioritize focus over required empty
    if (required && (!currentValue || currentValue.trim() === '')) return TEXTFIELD_STATES.REQUIRED_EMPTY;
    return TEXTFIELD_STATES.DEFAULT;
  };

  const visualState = getVisualState();
  const hasValue = currentValue && currentValue.trim() !== '';
  const colors = getTextFieldColors(visualState, theme, isFocused, isHovered, hasValue);
  ...
  const textFieldId = id || `textfield-${Math.random().toString(36).substr(2, 9)}`;
  const helperId = helperText ? `${textFieldId}-helper` : undefined;
  const errorId = errorMessage ? `${textFieldId}-error` : undefined;
  const successId = successMessage ? `${textFieldId}-success` : undefined;
...
TextField.propTypes = {
  ...
  theme: PropTypes.oneOf(Object.values(TEXTFIELD_THEMES)),
  state: PropTypes.oneOf(Object.values(TEXTFIELD_STATES)),
  id: PropTypes.string,
