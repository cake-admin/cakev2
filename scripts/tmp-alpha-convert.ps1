...
function Convert-BlackToAlpha {
  param([string]$InPath, [string]$OutPath, [int]$Threshold = 8)
  ...
  # color type at IHDR offset 25 (byte after width/height/bitdepth)
  $colorType = $bytes[25]
  Write-Output ("OUT={0} cleared={1} size={2} isPng={3} colorType={4}" -f $OutPath, $cleared, $bytes.Length, $isPng, $colorType)
}

$light = "C:\Users\chris_f40v4nt\Documents\GitHub\cakev2\src\assets\home\win-canvas-light.png"
$dark = "C:\Users\chris_f40v4nt\Documents\GitHub\cakev2\src\assets\home\win-canvas-dark.png"
$tmpL = Join-Path $env:TEMP "win-canvas-light-alpha.png"
$tmpD = Join-Path $env:TEMP "win-canvas-dark-alpha.png"

Convert-BlackToAlpha -InPath $light -OutPath $tmpL
Convert-BlackToAlpha -InPath $dark -OutPath $tmpD
Copy-Item -Force $tmpL $light
